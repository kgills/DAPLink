...
void stdio_init(void)
{
    ...
    MXC_CLKMAN->clk_gate_ctrl1 |= (2 << MXC_F_CLKMAN_CLK_GATE_CTRL1_UART0_CLK_GATER_POS);
    ...
    MXC_UART0->ctrl &= ~(MXC_F_UART_CTRL_RX_FIFO_EN | MXC_F_UART_CTRL_TX_FIFO_EN);
    MXC_UART0->ctrl |= (MXC_F_UART_CTRL_RX_FIFO_EN | MXC_F_UART_CTRL_TX_FIFO_EN);

    // Disable interrupts
    MXC_UART0->inten = 0;
    MXC_UART0->intfl = MXC_UART0->intfl;

    // Set the parity, size, stop and flow configuration
    MXC_UART0->ctrl |= (MXC_S_UART_CTRL_DATA_SIZE_8_BITS | MXC_S_UART_CTRL_PARITY_DISABLE);

    // Set the baud rate
    set_baudrate();

    MXC_UART0->ctrl |= MXC_F_UART_CTRL_UART_EN;
}
...
int fputc(int ch, FILE *f)
{
    if (ch == '\n') {
        ...
        while ((MXC_UART0->tx_fifo_ctrl & MXC_F_UART_TX_FIFO_CTRL_FIFO_ENTRY) == MXC_F_UART_TX_FIFO_CTRL_FIFO_ENTRY);
        MXC_UART0_FIFO->tx = '\r';
    }

    // Wait for TXFIFO to not be full
    while ((MXC_UART0->tx_fifo_ctrl & MXC_F_UART_TX_FIFO_CTRL_FIFO_ENTRY) == MXC_F_UART_TX_FIFO_CTRL_FIFO_ENTRY);
    MXC_UART0_FIFO->tx = ch;
...
int fgetc(FILE *f)
{
    ...
    while (!(MXC_UART0->rx_fifo_ctrl & MXC_F_UART_RX_FIFO_CTRL_FIFO_ENTRY));

    ch = MXC_UART0_FIFO->rx & (uint8_t)0xFF;
    fputc(ch, f);
