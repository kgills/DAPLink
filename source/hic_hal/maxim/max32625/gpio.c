...
void gpio_set_hid_led(uint8_t state)
{
    if (state == GPIO_LED_OFF) {
        MXC_CLRBIT(&MXC_GPIO->out_val[LED_PORT], DAP_LED_PIN);
    } else {
        MXC_SETBIT(&MXC_GPIO->out_val[LED_PORT], DAP_LED_PIN);
    }
}
...
{
    if (state == GPIO_LED_OFF) {
        MXC_CLRBIT(&MXC_GPIO->out_val[LED_PORT], MSD_LED_PIN);
    } else {
        MXC_SETBIT(&MXC_GPIO->out_val[LED_PORT], MSD_LED_PIN);
    }
}
...
{
    if (state == GPIO_LED_OFF) {
        MXC_CLRBIT(&MXC_GPIO->out_val[LED_PORT], CDC_LED_PIN);
    } else {
        MXC_SETBIT(&MXC_GPIO->out_val[LED_PORT], CDC_LED_PIN);
    }
}
...
{
    return MXC_GETBIT(&MXC_GPIO->in_val[BUTTON_PORT], BUTTON_PIN);
}
