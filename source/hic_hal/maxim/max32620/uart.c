...
#define CDC_ACM_UART 0

/* Concatenation macros  */
#define CC1(a, b) CC2(a, b)
#define CC2(a, b) a ## b
#define CC3(a, b, c) CC4(a, b, c)
#define CC4(a, b, c) a ## b ## c
#define CDC_ACM_MXC_UART CC1(MXC_UART, CDC_ACM_UART)
#define CDC_ACM_MXC_UART_FIFO CC3(MXC_UART, CDC_ACM_UART, _FIFO)
#define CDC_ACM_MXC_UART_IRQ_NUM CC3(UART, CDC_ACM_UART,_IRQn)
...
static mxc_uart_regs_t *CdcAcmUart = (mxc_uart_regs_t*)CDC_ACM_MXC_UART;
static mxc_uart_fifo_regs_t *CdcAcmUartFifo = (mxc_uart_fifo_regs_t*)CDC_ACM_MXC_UART_FIFO;
static const IRQn_Type CdcAcmUartIrqNumber = CDC_ACM_MXC_UART_IRQ_NUM;
...
int32_t uart_initialize(void)
{
  ...
  // Configure GPIO for UART
	MXC_IOMAN->uart3_req = ((MXC_E_IOMAN_MAPPING_B << MXC_F_IOMAN_UART3_REQ_IO_MAP_POS) | MXC_F_IOMAN_UART3_REQ_IO_REQ);
	while (MXC_IOMAN->uart3_ack != ((MXC_E_IOMAN_MAPPING_B << MXC_F_IOMAN_UART3_REQ_IO_MAP_POS) | MXC_F_IOMAN_UART3_REQ_IO_REQ));
#elif (CDC_ACM_UART == 0)
  ...
  // Configure GPIO for UART
	MXC_IOMAN->uart0_req = ((MXC_E_IOMAN_MAPPING_A << MXC_F_IOMAN_UART0_REQ_IO_MAP_POS) | MXC_F_IOMAN_UART0_REQ_IO_REQ);
	while (MXC_IOMAN->uart0_ack != ((MXC_E_IOMAN_MAPPING_A << MXC_F_IOMAN_UART0_REQ_IO_MAP_POS) | MXC_F_IOMAN_UART0_REQ_IO_REQ));
#else
	#error Unsupported CDC/ACM UART
#endif
...
//	CdcAcmUart->inten = (MXC_F_UART_INTEN_RX_FIFO_AF | MXC_F_UART_INTEN_TX_FIFO_AE);
...
#if defined(TARGET_MAX32620_HDK) || defined(TARGET_MAX32620_MBED)
		                                  return 0;
#else
                                      ctrl |= (MXC_F_UART_CTRL_CTS_EN | MXC_F_UART_CTRL_RTS_EN); break;
#endif
...
//	if (intfl & (MXC_F_UART_INTFL_RX_FIFO_AF | UART_ERRORS)) {
	if (intfl & (MXC_F_UART_INTFL_RX_FIFO_NOT_EMPTY | UART_ERRORS)) {
		while ((CdcAcmUart->rx_fifo_ctrl & MXC_F_UART_TX_FIFO_CTRL_FIFO_ENTRY) &&
