...
static __inline void PORT_JTAG_SETUP (void) {
  ...
  MXC_GPIO->in_mode[TRGT_PORT] &= ~(0xF << (4 * TDO_PIN));
  MXC_GPIO->in_mode[TRGT_PORT] &= ~(0xF << (4 * VDDIO_PIN));
...
static __inline void PORT_OFF (void) {
  ...
  out_mode |= (MXC_E_GPIO_OUT_MODE_TRISTATE << (4 * VDDIO_PIN));
  MXC_GPIO->out_mode[TRGT_PORT] = out_mode;

  MXC_GPIO->out_val[BUFFEN_PORT] = 0x0;  // Disable the buffer
...
static __forceinline uint32_t PIN_SWCLK_TCK_IN  (void) {
    return BITBAND_GetBit(&MXC_GPIO->in_val[TRGT_PORT], TCK_PIN);
}
...
static __forceinline void     PIN_SWCLK_TCK_SET (void) {
  BITBAND_SetBit(&MXC_GPIO->out_val[TRGT_PORT], TCK_PIN);
}
...
static __forceinline void     PIN_SWCLK_TCK_CLR (void) {
  BITBAND_ClrBit(&MXC_GPIO->out_val[TRGT_PORT], TCK_PIN);
}
...
static __forceinline uint32_t PIN_SWDIO_TMS_IN  (void) {
  return BITBAND_GetBit(&MXC_GPIO->in_val[TRGT_PORT], TMS_PIN);
}
...
static __forceinline void     PIN_SWDIO_TMS_SET (void) {
  BITBAND_SetBit(&MXC_GPIO->out_val[TRGT_PORT], TMS_PIN);
}
...
static __forceinline void     PIN_SWDIO_TMS_CLR (void) {
  BITBAND_ClrBit(&MXC_GPIO->out_val[TRGT_PORT], TMS_PIN);
}
...
static __forceinline uint32_t PIN_SWDIO_IN      (void) {
 return BITBAND_GetBit(&MXC_GPIO->in_val[TRGT_PORT], SWDIO_PIN);
}
...
static __forceinline void     PIN_SWDIO_OUT     (uint32_t bit){
  if (bit) {
    BITBAND_SetBit(&MXC_GPIO->out_val[TRGT_PORT], SWDIO_PIN);
  } else {
    BITBAND_ClrBit(&MXC_GPIO->out_val[TRGT_PORT], SWDIO_PIN);
  }
}
...
static __forceinline uint32_t PIN_TDI_IN  (void) {
    return BITBAND_GetBit(&MXC_GPIO->in_val[TRGT_PORT], TDI_PIN);
}
...
static __forceinline void     PIN_TDI_OUT (uint32_t bit) {
  if (bit & 1) {
    BITBAND_SetBit(&MXC_GPIO->out_val[TRGT_PORT], TDI_PIN);
  } else {
    BITBAND_ClrBit(&MXC_GPIO->out_val[TRGT_PORT], TDI_PIN);
  }
}
...
static __forceinline uint32_t PIN_TDO_IN  (void) {
    return BITBAND_GetBit(&MXC_GPIO->in_val[TRGT_PORT], TDO_PIN);
}
...
static __forceinline uint32_t PIN_nRESET_IN  (void) {
    return BITBAND_GetBit(&MXC_GPIO->in_val[TRGT_PORT], SRST_PIN);
}
...
static __forceinline void     PIN_nRESET_OUT (uint32_t bit) {
  if (bit) {
    BITBAND_SetBit(&MXC_GPIO->out_val[TRGT_PORT], SRST_PIN);
  } else {
    BITBAND_ClrBit(&MXC_GPIO->out_val[TRGT_PORT], SRST_PIN);
  }
}
...
static __inline void DAP_SETUP (void) {
  ...
  out_mode = MXC_GPIO->out_mode[BUFFEN_PORT];
  out_mode &= ~(0xF << (4 * BUFFEN_PIN));
  out_mode |= (MXC_E_GPIO_OUT_MODE_TRISTATE << (4 * BUFFEN_PIN));
  MXC_GPIO->out_mode[BUFFEN_PORT] = out_mode;
}
